...
{
				   var userName = $("input[name=subjectUserName]").val();
				 ...
				 // console.log(data);
				   ...
				   $.post(
						"/Admin/Index/createNewUser",
						 { "username": userName, 'sex':sex, 'age':age, 'desc':desc, 'birth':birth, "phone":phone, "email":email},
						function( data ) {
							...
							var add = data.add;
								if(Boolean(add))
								{
									$('#user-info').html("新用户创建成功！");
								}
								if(Boolean(update))
								{
									$('#user-info').html("用户信息已经更新成功！");
								}
						}, 
						"json"
				   );
